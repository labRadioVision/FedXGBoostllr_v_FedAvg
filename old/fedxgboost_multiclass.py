...
def rounded_accuracy(y_true, y_pred):
  """
  Custom TensorFlow metric to compute accuracy using rounded predictions.
  ...
  Returns:
      Tensor: Accuracy (percentage of correctly classified samples) after rounding predictions.
  """

  # Round predictions to the nearest integer
  rounded_preds = K.round(y_pred)

  # Cast both ground truth and rounded predictions to integer type for comparison
  y_true = K.cast(y_true, dtype='int32')
  rounded_preds = K.cast(rounded_preds, dtype='int32')

  # Calculate accuracy as the mean of equal elements
  accuracy = K.mean(K.equal(y_true, rounded_preds))

  # Return accuracy as a percentage
  return accuracy, rounded_preds
...
# a_s, rounded_y_pred = rounded_accuracy(y_valid, y_pred)
accuracy_s = accuracy_score(y_valid, rounded_y_pred)
print(f"Accuracy: {accuracy_s:.2f}") 

cm = pd.DataFrame(confusion_matrix(y_valid, rounded_y_pred)).to_numpy()
